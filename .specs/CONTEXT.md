...
| DocumentRepository JPQL | `findByIdAndIsDeletedFalse` 等逻辑删除查询使用 `@Query("... WHERE d.isDeleted = 0")` 显式 JPQL，避免 Hibernate 6.5 对 `Boolean` + MySQL TINYINT 的 `isFalse()` 谓词构建 bug | 2026-06-14 | `knowledge-graph-extraction` DEV |
...
- Git提交时不设置共同创作者 Commit without co-author
